...
public final class A2dpSinkHelper {
     ...
     */
    static final int A2DP_SINK_PROFILE = 11;
     ...
     * receive.
     */
    static final String ACTION_CONNECTION_STATE_CHANGED =
     ...
     * receive.
     */
    static final String ACTION_PLAYING_STATE_CHANGED =
    ...
    static final Integer AVRCP_CONTROLLER_PROFILE = 33;

    static int getPreviousAdapterState(Intent intent) {
        return intent.getIntExtra(BluetoothAdapter.EXTRA_PREVIOUS_STATE, -1);
    }

    static int getCurrentAdapterState(Intent intent) {
        return intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, -1);
    }

    static int getPreviousProfileState(Intent intent) {
        return intent.getIntExtra(BluetoothProfile.EXTRA_PREVIOUS_STATE, -1);
    }

    static int getCurrentProfileState(Intent intent) {
        return intent.getIntExtra(BluetoothProfile.EXTRA_STATE, -1);
    }

    static BluetoothDevice getDevice(Intent intent) {
     ...
     * @return boolean
     */
    static boolean disconnect(BluetoothProfile profile, ArrayList<BluetoothDevice> device) {
        try {
